...
class ClienteListCreateView(generics.ListCreateAPIView):
    """
    GET  /api/clientes/          — lista paginada (RF010)
    GET  /api/clientes/?search=  — pesquisa por nome, e-mail ou telefone (RF010)
    POST /api/clientes/          — cadastra novo cliente (RF004)
    """
    permission_classes = [IsAuthenticated]
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Retorna o objeto completo (com contadores) após criação
...
class ClienteDetailView(generics.RetrieveUpdateDestroyAPIView):
    """
    GET    /api/clientes/{id}/  — detalha cliente (RF009)
    PUT    /api/clientes/{id}/  — edição completa (RF005)
    PATCH  /api/clientes/{id}/  — edição parcial (RF005)
    DELETE /api/clientes/{id}/  — exclui se sem OS ativa (RF006 / RN001)
    """
    permission_classes = [IsAuthenticated]
    ...
    def update(self, request, *args, **kwargs):
        ...
        cliente = serializer.save()
        read_serializer = ClienteSerializer(
            self.get_queryset().get(pk=cliente.pk)
        )
        return Response(read_serializer.data)

    def destroy(self, request, *args, **kwargs):
        cliente = self.get_object()
        # RN001: bloqueia exclusão se há OS ativa (status != encerrada)
        if cliente.tem_os_ativa:
            return Response(
                {'error': 'Este cliente possui chamados em aberto e não pode ser excluído. '
                          'Encerre todas as OS antes de excluí-lo. (RN001)'},
                status=status.HTTP_400_BAD_REQUEST
            )
        # Remove OS encerradas antes de excluir o cliente (FK PROTECT)
        cliente.ordens.filter(status='encerrada').delete()
        cliente.delete()
