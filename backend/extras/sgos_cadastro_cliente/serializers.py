...
class ClienteSerializer(serializers.ModelSerializer):
    """Serializer de leitura — inclui contadores de OS."""
    ...
    class Meta:
        ...
        fields = (
            ...
            'criado_em', 'atualizado_em', 'total_os', 'os_abertas', 'tem_os_ativa',
        )
        read_only_fields = ('id', 'criado_em', 'atualizado_em')

    def get_total_os(self, obj):
        return getattr(obj, 'total_os', obj.ordens.count())

    def get_os_abertas(self, obj):
        return getattr(obj, 'os_abertas', obj.ordens.exclude(status='encerrada').count())


class ClienteCreateUpdateSerializer(serializers.ModelSerializer):
    """
    Serializer de escrita (POST / PUT / PATCH).
    Valida: nome mínimo, telefone formato BR, e-mail único por instância.
    """
    class Meta:
        model  = Cliente
    ...
    def validate_nome(self, value):
        ...
        if not _re.match(r"^[A-Za-z\u00C0-\u00FF0-9 '\-\.]+$", value):
    ...
    def validate_email(self, value):
        ...
        if qs.exists():
            raise serializers.ValidationError('Já existe um cliente cadastrado com este e-mail.')
        return value
